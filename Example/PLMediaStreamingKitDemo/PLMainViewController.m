...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    effectConfiguration.licenseFilePath = [NSString pathWithComponents:@[rootPath, @"LicenseBag.bundle", @"qiniu_20200214_20210213_com.qbox.PLMediaStreamingKit.ByteDance.Demo_qiniu_v3.4.2.licbag"]];
...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    [_effectManager updateSticker:[[PLSEffectModel alloc] init]];
    [_effectManager updateFilterIntensity:0];
    [_effectManager updateFilter:[[PLSEffectModel alloc] init]];
    [_effectManager updateMakeupComponents:[[NSArray alloc] init]];
...
- (void)_prepareButtons
{
    ...
    self.stickerButton = ({
        UIButton *button = [UIButton buttonWithType:UIButtonTypeSystem];
        [self.cameraPreviewView addSubview:button];
        [button setTitle:@"贴纸" forState:UIControlStateNormal];
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(100, 50));
            make.bottom.equalTo(self.cameraPreviewView).with.offset(-240);
            make.centerX.equalTo(self.cameraPreviewView).with.offset(-120);
        }];
        button;
    });
    ...
    self.effectButton = ({
        UIButton *button = [UIButton buttonWithType:UIButtonTypeSystem];
        [self.cameraPreviewView addSubview:button];
        [button setTitle:@"特效" forState:UIControlStateNormal];
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(100, 50));
            make.bottom.equalTo(self.cameraPreviewView).with.offset(-240);
            make.centerX.equalTo(self.cameraPreviewView).with.offset(120);
    ...
    [_stickerButton addTarget:self action:@selector(stickerButtonDidClick:) forControlEvents:UIControlEventTouchUpInside];
    [_effectButton addTarget:self action:@selector(effectButtonDidClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (CVPixelBufferRef)panelDelegateGenerator:(PLPanelDelegateGenerator *)panelDelegateGenerator cameraSourceDidGetPixelBuffer:(CVPixelBufferRef)pixelBuffer timingInfo:(CMSampleTimingInfo)timingInfo {
